...
if (app.isPackaged) {
  app.setPath('userData', path.join(app.getPath('appData'), 'Mercado Livre Gestao'));
}
...
function dataPath(...parts) {
  const baseDir = app.isPackaged ? app.getPath('userData') : rootDir;
...
function createWindow() {
  mainWindow = new BrowserWindow({
    ...
    resizable: true,
    maximizable: true,
    icon: path.join(rootDir, 'img', 'mercado-livre.png'),
  ...
  const startPage = path.join(__dirname, '..', '..', 'pages', 'inicio.html');
...
ipcMain.handle('notas-fiscais:save', (_event, payload) => {
  ...
  const dir = dataPath('Banco de Dados', 'notas-fiscais');
...
ipcMain.handle('imagens-anuncios:save', (_event, payload) => {
  ...
  const dir = dataPath('assets', 'uploads', 'anuncios');
  fs.mkdirSync(dir, { recursive: true });
